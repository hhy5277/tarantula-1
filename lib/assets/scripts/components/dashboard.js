                  ...
                  <template v-for="(item, index) in items">
                    <v-list-tile avatar :key="item.name">
                      ...
                      <div class="align-center" style="margin-left: auto" v-for="control in item.control">
                        <v-btn icon @click="control.click(item, index)" :title="$t(control.title)" :disabled="control.disabled" v-if="!control.isHidden">
  ...
  methods: {
    clickProjectPlay: function(v){
      startProject(v.id).then((r) => {
        _.chain(this.items)
         .find((n) => r.data.id == n.id)
         .tap((n) => _.merge(n, r.data))
         .value()
      })
    },
    ...
    clickConfirmDelete: function(){
      ...
      deleteProject(this.currentItem)
    },

    clickProjectReset: function(v, i){
      resetProject(v.id)
      //_.find(v.control, (n) => n.icon == 'fa-undo').disabled = true
      //this.items.splice(i, v)
    },
