...
const Dashboard = {
                    ...
                    <v-list-tile avatar :key="item.name">
                      ...
                      <div class="align-center" style="margin-left: auto">
                        <v-btn icon disabled v-if="item.cron" :title="$t('dashboard.btnCron')">
                          <v-icon color="blue-grey darken-2 dashborad-btn">alarm</v-icon>
                        </v-btn>

                        <v-btn icon @click="clickProjectPlay(item)" :title="$t('dashboard.btnPlay')">
                          <v-icon class='dashborad-btn' color="blue-grey darken-2">fa-play</v-icon>
                        </v-btn>

                        <v-btn icon @click="clickProjectTable(item)" :title="$t('dashboard.btnTable')">
                          <v-icon class='dashborad-btn' color="blue-grey darken-2">fa-table</v-icon>
                        </v-btn>

                        <v-btn icon @click="clickProjectStats(item)" :title="$t('dashboard.btnStats')">
                          <v-icon class='dashborad-btn' color="blue-grey darken-2">fa-bar-chart</v-icon>
                        </v-btn>

                        <v-btn icon @click="clickDeleteButton(item)" :title="$t('dashboard.btnDelete')">
                          <v-icon class='dashborad-btn' color="blue-grey darken-2">fa-trash-o</v-icon>
                        </v-btn>
                      </div>
  ...
  methods: {
    clickProjectPlay: function(v){
      startProject(v.id).then((r) => {
        _.chain(this.items).find((n) => r.data.id == n.id).tap((n) => _.merge(n, r.data)).value()
      })
    },
