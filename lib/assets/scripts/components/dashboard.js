...
const Dashboard = {
  template: `
    <div>
      ...
      <v-dialog v-model="dialogDelete" max-width="360">
       <v-card>
         <v-card-title class="headline">{{ $t("dashboard.deleteDialog.title", {name: _.get(selectedProject, "name")}) }}</v-card-title>
  ...
  computed: {
    ...Vuex.mapGetters(['dashbordProjects', 'selectedProject']),
  ...
  methods: {
    ...Vuex.mapActions(['actionSelectProject', 'actionStartProject', 'actionDeleteProject', 'actionResetProject']),
