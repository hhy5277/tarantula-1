...
export default {
  ...
  actions: {
    ...
    actionResetProject({commit, dispatch, state, getters}, {index, item}){
      commit(types.PROJECT_TOGGLE_STATUS, {index, key: 'undo'})
    ...
    async actionGetMessage({commit, dispatch, getters}){
      const r = await _.map(getters.dashbordProjects, async (item) => {
        const { messages } = await fetchProjectMessage(item.id)

        return _.map(messages, (n) => {
          return _.merge(n, {"@createdAt": moment(n['@createdAt'], "x").format("YYYY-MM-DD HH:mm:ss")})
        })
      })
    ...
    actionGetProcessData({commit, dispatch}, id){
      return fetchProjectData(id)
    }
  },
